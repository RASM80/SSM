...
RECOGNITION_RESOLUTION = (960, 540)
FPS = 10
...
def main():
    ...
    # Initialize last authorization time
    last_auth_time = {}
    ...
    while not quit_flag:
        ...
        for frame_number in range(SESSION_FRAMES):
            ...
            for face_index, ((top, right, bottom, left), face_encoding) in enumerate(zip(full_hd_locations, face_encodings)):
                ...
                # Compute landmarks for EAR
                face_landmarks = face_recognition.face_landmarks(recognition_frame, [rec_locations[face_index]])
                if face_landmarks:
                    ...
                    if "left_eye" in landmarks and "right_eye" in landmarks:
                        ...
                        if name != "Unknown":
                            if name not in session_data:
                                session_data[name] = {'recognition_count': 0, 'last_counted_frame': -SKIP_FRAMES - 1, 'ear_data': []}
                            session_data[name]['ear_data'].append((frame_number, ear))
                            if frame_number > session_data[name]['last_counted_frame'] + SKIP_FRAMES:
                                session_data[name]['recognition_count'] += 1
                                session_data[name]['last_counted_frame'] = frame_number
        ...
        # Process authorization after session
        current_time = time.time()
        for name in session_data:
            if session_data[name]['recognition_count'] >= REQUIRED_RECOGNITIONS:
                if has_blink(session_data[name]['ear_data'], CONSECUTIVE_FRAMES, EAR_THRESHOLD):
                    last_auth = last_auth_time.get(name, 0)
                    if current_time - last_auth > MIN_AUTH_INTERVAL:
